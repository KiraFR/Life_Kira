...
_CheckEntreprise = "SELECT entreprise FROM banque"
_ResultCheckNum = [_CheckNum,2,true] call DB_fnc_asyncCall;
_Entreprise = if(_CheckEntreprise = 1) exitwith {systemChat "Tu as deja un compte Entreprise tu ne peut en avoir qu'un"}
_deja = _Entreprise;
{	
...
if(_deja) then{"Vous avez deja un compte Entreprise" remoteExec ["hint",(owner _owner)];}else{
_insertNum = format["INSERT INTO banque (playerid,numcompte,offshore , entreprise) VALUES('%1', '""%2""','0', '1')",_uid,_numcompte,_offshore];
