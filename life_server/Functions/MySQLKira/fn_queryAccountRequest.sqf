...
_total = [];
_cpt = 0;
_accNormal = format["SELECT nam_account,numcompte,bankacc,dflt FROM banque WHERE playerid='%1' AND ((offshore ='0' AND entreprise='0') OR epargne ='1');",_uid];
...
_accEntreprise = format["SELECT nam_account,numcompte,bankacc,dflt FROM banque WHERE playerid='%1' AND entreprise='1';",_uid];
...
_accOffShore = format["SELECT nam_account,numcompte,bankacc,dflt FROM banque WHERE playerid='%1' AND offshore ='1';",_uid];
