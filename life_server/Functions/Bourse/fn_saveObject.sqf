                ...
                _Object_Bourse set [_id,_newValueArray];
                {
                    ...
                    _valueModif = _count * _cofi;
                    {
                        if (_name == (_x select 0)) then {
                            ...
                            diag_log format["Relation ==> Resource : %2 | %1",_valueArray,_name];
                            _id = _Object_Bourse find _ValueArray;
                            diag_log format["Relation ==> Resource : %2 | %1",_id,_name];
                            ...
                            if (_NewValueFor >= _maxFor) then {
                                _valueNew = _maxFor;
                                diag_log format["Relation ==> Resource : %2 | %1 >= %3",_newValueArray,_name,_maxFor];
                            };

                            if (_NewValueFor <= _minFor) then {
                                _valueNew = _minFor;
                                diag_log format["Relation ==> Resource : %2 | %1 <= %3",_newValueArray,_name,_minFor];
                            };

                            _newValueArray = [_name,(_NewValueFor)];
                            diag_log format["Relation ==> Resource : %2 | %1",_newValueArray,_name];
                            _Object_Bourse set [_id,_newValueArray];
                        };
